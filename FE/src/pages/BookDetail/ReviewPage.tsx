...
import {
  ...
  updateReview,
  fetchReviews,
} from '../../services/BookDetail/BookReview';
...
const ReviewPage = () => {
  ...
  const renderReviewInput = () => (
    <View style={styles.reviewInputContainer}>
      <Text style={styles.ratingLabel} accessibilityLabel="별점 입력">
        평점
      </Text>
      ...
      <TouchableOpacity
        style={[
          styles.submitButton,
          newReviewScore === 0 && styles.submitButtonDisabled,
        ]}
  ...
  const renderMemberReview = () => (
    <View style={styles.memberReviewContainer}>
      <View
        ...
        accessibilityLabel={`나의 리뷰 별점: ${memberReview!.score}점`}
        accessible={true}
      ...
      <Text style={[styles.memberReviewContent]}>
        {expandedReviews.has(memberReview!.reviewId)
          ...
          : memberReview!.content.slice(0, 100).replace(/ /g, '\u00A0')}
      ...
      <Text
        ...
        accessibilityLabel={`최종 작성일 ${memberReview.updatedAt}`}>
        {memberReview!.updatedAt}
      </Text>
        ...
        renderItem={({item}) => (
          <View style={styles.reviewItem}>
            ...
            <View
              ...
              accessible={true}
              accessibilityLabel={`별점 ${item.score}점`}
              accessibilityRole="text">
              {[...Array(5)].map((_, index) => (
                <Text
                  key={index}
                  style={
                    index < item.score ? styles.starFilled : styles.starEmpty
                  }
            ...
            <Text style={styles.reviewContent}>
              {expandedReviews.has(item.reviewId)
                ? item.content
            ...
            <Text
              style={styles.updatedAt}
              accessibilityLabel={`최종 작성일 ${item.updatedAt}`}>
              {item.updatedAt}
            </Text>
          </View>
        )}
