...
const MyBooksPage: React.FC = () => {
  const [isAccessibilityMode, setIsAccessibilityMode] =
    useState<boolean>(false); // 접근성 모드
  const [selectedTab, setSelectedTab] = useState<'출판도서' | '등록도서'>(
    '출판도서',
  );
  ...
  useEffect(() => {
    let filtered = allBooks.filter(book =>
      selectedTab === '출판도서'
        ? book.dtype === 'PUBLISHED'
        : book.dtype === 'REGISTERED',
    );
