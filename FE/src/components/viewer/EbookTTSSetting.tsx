...
const EbookTTSSetting: React.FC<SidebarProps> = ({
  settingTTSSideBarX,
  toggleTTSSetting,
  handleTimerOn,
}) => {
  ...
  const [isVoiceSelectModalOpen, setIsVoiceSelectModalOpen] =
  ...
  return (
    <Animated.View style={[styles.sidebar, animatedIndexStyle]}>
      ...
      <ScrollView style={{flex: 1, marginTop: height * 0.1}}>
        <IndexChapter
          chapter={'TTS 속도'}
          onPress={() => setisSpeedModalOpen(!isSpeedModalOpen)}
        />
        <IndexChapter
          chapter={'TTS 보이스'}
          onPress={() => setIsVoiceSelectModalOpen(!isVoiceSelectModalOpen)}
        />
        <IndexChapter
          chapter={'TTS 타이머'}
          onPress={() => setIsTimerModalOpen(!isTimerModalOpen)}
        />
      </ScrollView>
      {isSpeedModalOpen ? (
        <SpeedModal closeModal={handleSpeedModalClose} />
      ) : null}
      {isVoiceSelectModalOpen ? (
        <VoiceSelectModal closeModal={handleVoiceSelectModalClose} />
      ) : null}
      {isTimerModalOpen ? (
        <TimerModal
          closeModal={handleTimerModalClose}
